...
const Faculty: React.FC = () => {
    ...
    useEffect(() => {
        const fetchQuizzes = async () => {
            const facultyID = localStorage.getItem('facultyID');
            try {
                const response = await axios.get(`https://localhost:7116/api/Quiz/getQuiz/${facultyID}`);
                setQuizzes(response.data);
            } catch (error) {
                setErrorMessage('Error occurred while fetching quizzes.');
                console.error('Error fetching quizzes:', error);
    ...
    const openUpdateQuizPage = (quizId: string) => {
        navigate(`/faculty/updatequiz/${quizId}`);
    };
                        ...
                        <div className="col-md-4 mb-4" key={quiz.id}>
                            <div className="card h-100 shadow">
                                ...
                                <div className="card-footer">
                                    <button
                                        className="btn btn-primary"
                                        onClick={() => openUpdateQuizPage(quiz.quizId)}
                                    >
                                        Update Quiz
                                    </button>
                                    <button
